...
for kernel_name, kernel in {'gaussian': utils.kernel_gaussian, 'polynomial': utils.kernel_polynomial}.items():
    for kernel_param in [0.5, 1, 2]:
        exp_results_details = pd.DataFrame()

        #:# d=1/2 returns a corset consisting of one class only, which breaks pvi
        if kernel_name == "polynomial" and kernel_param == 1/2:
            kernel_param = 3
        ...
        exp_results_details.to_csv(f'results/exp4_nocat_{kernel_name}_{kernel_param}.csv', index=False)
...
exp_results_summary.to_csv(f'results/exp4_nocat_summary.csv')