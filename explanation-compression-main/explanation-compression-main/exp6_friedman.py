...
X, y = make_friedman1(n_samples=10000,noise=0.1)

model = xgboost.XGBRegressor(max_depth=2, subsample=0.9**3, colsample_bytree=0.9, colsample_bylevel=0.9, colsample_bynode=0.9, alpha=0.1)
model.fit(X, y)


shap_exp = shap.explainers.Tree(model, data=X, model_output="raw")
...
for seed in tqdm(range(101)):

    t.tic()
    id_compressed = kt.thin(X, m=3, split_kernel=utils.kernel_gaussian, swap_kernel=utils.kernel_gaussian, seed=seed)
    t.toc()
    time_kt = t.elapsed
    ...
    shap_exp_compressed = shap.explainers.Tree(model, data=X_compressed, model_output="raw")
    t.tic()
    ...
    shap_exp_random = shap.explainers.Tree(model, data=X_random, model_output="raw")
    ...
    exp_results = pd.concat([exp_results, pd.DataFrame({
        'model_performance': exp.model_performance().result.r2.values[0],
        'wd_random': wd_random,#wasserstein distance between true distribution and  randomly sampled points
        'wd_compressed': wd_compressed,
        'wd_diff': wd_random - wd_compressed,
        'svi_random': np.sum(np.abs(shap_svi - shap_svi_random)),#sum of shapley values - why???
        ...
        'sv_wd_random': sv_wd_random,#wasserstein distance between shap on true distribution and shap on randomly sampled points
